...
class MainActivity : AppCompatActivity() {
    ...
    private fun ActivityMainBinding.initView() {
        ...
        if (pickPictureFromFilesystem){
            inputBtnImg.icon = ContextCompat.getDrawable(this@MainActivity, R.drawable.baseline_attach_file_24)
        }

        if (useBuiltinCamera){
            inputBtnImg.icon = ContextCompat.getDrawable(this@MainActivity, R.drawable.ic_baseline_add_a_photo_24)
        }
    ...
    private fun ActivityMainBinding.setUpObserver() {
        calculatorViewModel.apply {
            ...
            imageResultUri.observe(this@MainActivity) { uri ->
            }
        }
    }
    ...
    private fun recognizeTextFromImage(){
        ...
        try {
            ...
            calculatorViewModel.apply {
                val inputImage = InputImage.fromFilePath(this@MainActivity, imageResultUri.value!!)
    ...
    private fun permissionStorage(){
        lifecycleScope.launch {
            val storagePermissionGranted = withContext(Dispatchers.Main) { this@MainActivity.requestStoragePermission() }
    ...
    private fun permissionCamera(){
        lifecycleScope.launch {
            val cameraPermissionGranted = withContext(Dispatchers.Main) { this@MainActivity.requestCameraPermission() }
